 ...
 */
export default function screensPlugin({ storeName = "screens", initialScreen = null } = {}) {
    return (Alpine) => {
        Alpine.store(storeName, {
            ...
            changeScreen(screenName) {
                ...
                // Dispatch the "screen-exit" event on all of the elements of the current screen
                if (this.currentScreen && this.screenElements[this.currentScreen]) {
                    this.screenElements[this.currentScreen].forEach(el => el.dispatchEvent(new CustomEvent('screen-exit', { detail: { newScreen: screenName } })));
                }

                const oldScreen = this.currentScreen;
                this.currentScreen = screenName;

                // Dispatch the "screen-enter" event on all of the elements of the new screen
                if (this.screenElements[this.currentScreen]) {
                    this.screenElements[this.currentScreen].forEach(el => el.dispatchEvent(new CustomEvent('screen-enter', { detail: { oldScreen } })));
                }
            },
            screenElements: {}
        });
        ...
        Alpine.directive('screen', (el, { expression: screenName }, { effect, cleanup, Alpine }) => {
            ...
            cleanup(() => {
                ...
                if (store.screenElements[screenName].length === 0) {
                    delete store.screenElements[screenName];
                }
            });
